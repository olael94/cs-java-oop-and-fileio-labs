 ...
 * Description: Passing an ArrayList to sumValues() and creating dups() of list numbers. 
 */
...
public class Lab8b {
	...
	public static void main(String[] args) {
		...
		while(playAgain.equalsIgnoreCase("Y")) {
			...
			//playAgain message
			playAgain = getValidYN(sInt1, "\nWould you like to play again?: ", "Error! Please use Y for yes and N for no. \n");
	...
	//Beginning of dups() method
	public static ArrayList<Integer> dups(ArrayList<Integer> numbers) {
		ArrayList<Integer> duplicates = new ArrayList<Integer>();

		//We start by cycling through the "numbers" ArrayList. The variable current represents the current element in the numbers list.
		for (int i = 0; i < numbers.size(); i++) {

			//This will take whatever current number we input on the scanner and store it in a new variable
			int current = numbers.get(i);

			if (numbers.subList(i + 1, numbers.size()).contains(current) && !duplicates.contains(current)) {
            ...
            /* Before adding current to the duplicates list, we check if it's already there. 
            We use duplicates.contains(current) to check if the duplicates list already contains the current value.
            By using (!duplicate.contains (current)) we avoid adding the same duplicate value multiple times to the duplicates list
            if (!duplicates.contains(current)) {
                for (int j = i + 1; j < numbers.size(); j++) {

                	//If we find a duplicate value in numbers we will add it to duplicates
