...
public class Lab12b {
	...
	/** 1. addItems() method asks the user to enter an item to be stored in the shoppingList and then adds the item to the shoppingList ArrayList.
	...
	/**2. deleteItems() method asks the user to enter an item to be deleted from the shoppingList and then deletes the item from the shoppingList ArrayList.
	...
	/**3. showItems() method simply displays the contents of the shoppingList ArrayList in it's simplest form:
	 * Example output:
	 * 
	...
	/**4. sortItems() method sorts the items in the shoppingList ArrayList, then calls showItems() to display the sorted list.
	 * Example output:
	...
	//5. saveList() method to save objects to shopping list ADDED IN CLASS
	...
	//HELPER getChoice() method for Y and N. 
	...
	/* 6. openList method()
		When the user selects Open List from the menu, the program will ask if they are sure they want
		to overwrite the current shopping list being edited with the file on the fixed storage device
		and validate that the user responds with a Y or N.
	 */
	public static ArrayList<String> openList(Scanner sIn, ArrayList<String> shoppingList, String FILENAME ) {
		...
		if(confirmed.equalsIgnoreCase("Y")) {
			String filePath = "Shopping List.txt";
			File Shopping_List = new File(filePath);
			Scanner scanner = null;		//Create file Object

			try {
				scanner = new Scanner(Shopping_List);
				shoppingList.clear();
