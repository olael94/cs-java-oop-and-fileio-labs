...
/**
 * Class for an instructive program to teach debugging skills.
 * Contains the main method as well as auxiliary methods to utilize and debug a Java program. 
...
public class Lab11a {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case "4":
				...
				break;											//OLIVER RIVERA ---> I ADDED BREAK TO STOP RUNNING ANY CODE AFTER CASE 4 // Test Case 56 - Fail
			default:
	...
	public static void playMath(Scanner sIn, String game) {
		...
		if(game.contentEquals("sum")) {
			...
			result = biggestNumber(numList);					//Call the biggestNumber() method //OLIVER RIVERA ---> ADDED RESULT = "The variable wasn't being stored anywhere // Test Case 42- Fail
	...
	public static double sumNumbers(ArrayList<Double> numList) {
		...
		for (int i = 0; i < numList.size(); i++) {							//Process every number in the ArrayList// OLIVER RIVERA ---> DELETED "-1" to loop thru all the input lines. //Test Case 21 - Fail
	...
	public static double avgNumbers(ArrayList<Double> numList) {
		...
		for (int i = 0; i < numList.size(); i++) {								//Process every number in the ArrayList
			...
			System.out.printf("%,-15.2f%-15.2f\n",numList.get(i), (sum/(i+1)));	//Print the running average // OLIVER RIVERA ---> changed -15.2d to -15.2f "d" is for integers "f" is for doubles //Test Case 59 - Fail
		}//end of for (int i = 0; i < numList.size(); i++)
	...
	public static double biggestNumber(ArrayList<Double> numList) {
		...
		System.out.printf("%-15s%-15s%n", "Number", "Running Biggest");				//Print the header //OLIVER RIVERA ---> Removed\n and added %-12s%s\n
		...
		for (int i = 0; i < numList.size(); i++) {								//Process every number in the ArrayList
			...
			System.out.printf("%-15.2f%-15.2f%n", numList.get(i), result);		//Print the running average //OLIVER RIVERA ---> Removed , between % and -15.2f //Added 
		}//end of for (int i = 0; i < numList.size(); i++)	
